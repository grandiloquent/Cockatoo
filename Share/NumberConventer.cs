namespace Share
{
	...
	{
		private static string[] ArabinNum = {
			"0",
			"1",
			"2",
			"3",
        ...
        public static Decimal ChnToArab(string value)
        {
            ...
            //繁体数字替换处理
            string tempNumStr = "";
            for (int i = 0; i < ChnNumT.Length; i++)
            {
                value = value.Replace(ChnNumT[i], ChnNumS[i]);
            }
            //不规范/单个中文数字处理 Begin
            bool flag = System.Text.RegularExpressions.Regex.IsMatch(value, "^[零|一|二|三|四|五|六|七|八|九]*$");
            if (flag)
            {
                tempNumStr = value;
                for (int i = 0; i < 10; i++)
                {
                    tempNumStr = tempNumStr.Replace(ChnNumS[i], ArabinNum[i]);
                }
                if (tempNumStr.IsDigitString())
                {
                    return decimal.Parse(tempNumStr);
                }
            }
            //不规范中文数字处理 End
            ...
            else
            {
                if (yCount == 1)
                {
                    pre = pre.Replace("亿", ",");
                    string[] sp = pre.Split(',');
                    result = (neg ? -1 : 1) * ((HandlePart(sp[0]) * 100000000) + HandlePart(sp[1])) + dotPart;
                }
                else
                {
                    if (yCount == 0)
                    {
                        result = (neg ? -1 : 1) * HandlePart(pre) + dotPart;
                    }
                }
            }
            return result;
